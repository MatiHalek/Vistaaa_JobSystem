<?php
    ...
    {
        $result = $connect->execute_query("SELECT *, 1 AS available FROM advertisement WHERE company_id = ? AND date_expiration >= NOW() UNION SELECT *, 0 AS available FROM advertisement WHERE company_id = ? AND date_expiration < NOW() ORDER BY available DESC".$sort." LIMIT ".(($page - 1) * $offersPerPage).", ".$offersPerPage.";", [$_SESSION["logged"]["company_id"], $_SESSION["logged"]["company_id"]]);
    ...
    {
        $result = $connect->execute_query("SELECT *, 1 AS available FROM advertisement WHERE date_expiration >= NOW() UNION SELECT *, 0 AS available FROM advertisement WHERE date_expiration < NOW() ORDER BY available DESC".$sort." LIMIT ".(($page - 1) * $offersPerPage).",".$offersPerPage.";");
    ...
    if($result->num_rows > 0)
    {
        ...
        {
            echo "<div class='d-flex col-12 col-sm-6 col-lg-4 col-xl-3'><a class='d-block w-100 text-decoration-none bg-white shadow rounded position-relative p-3 pt-4 position-relative jobOffer ".($row["available"] == 0 ? " jobOfferDisabled" : "")."' href='offerdetails.php?id=".$row["advertisement_id"]."'>";
